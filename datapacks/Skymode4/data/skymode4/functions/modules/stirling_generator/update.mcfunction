...
execute if entity @s[tag=stirlingGeneratorActivated,scores={energy=985..}] run function skymode4:modules/stirling_generator/deactivate
execute if entity @s[tag=!stirlingGeneratorActivated,scores={energy=..920}] if block ~ ~ ~ furnace{Items:[{Slot:1b}]} unless block ~ ~ ~ furnace{Items:[{Slot:0b}]} run function skymode4:modules/stirling_generator/activate
