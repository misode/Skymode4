...
execute if entity @s[tag=mysticAttraction,scores={mysticPower=1..}] run function skymode4:modules/mystic_core/attraction_check

execute at @e[type=armor_stand,tag=mysticPlacer,distance=..8.1,scores={mysticPower=..99}] run function skymode4:modules/mystic_core/transfer_check
...
execute if entity @e[type=item,nbt={Item:{tag:{crystalAttraction:1b}}},distance=..0.5,limit=1,sort=nearest] run function skymode4:modules/mystic_core/crystal_attraction
